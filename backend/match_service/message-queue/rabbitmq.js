...
class RabbitMQService {
    ...
    async initialize() {
        try {
            ...
            if (rabbitmqUrl === undefined) {
                throw new Error(
                    "RABBITMQ_URL is not defined in the .env file."
                );
            }
            ...
            await this.channel.assertQueue(this.getQueue("cancelMatchmaking"), {
                durable: true,
            });
                ...
                async (message) => {
                    if (message) {
                        ...
                        for (
                            ...
                            const status = await this.channel?.checkQueue(
                                queue
                            );
                            console.log(status);
    ...
    async findMatch(player, matchQueue) {
        if (!this.channel) {
            console.error("RabbitMQ channel not initialized");
            return null;
        }
