...
async def get_all_question_status(userID: str):
  try:
    response = requests.get(config.question_status_service_url, params={"userID": userID})
...
async def update_question_status(status: QuestionStatus):
  try:
    response = requests.put(config.question_status_service_url, json=status.dict())
    return response.json()
  except Exception as e:
    return e